...
<Callout type="tip">
Groq 用 OpenAI-compatible API，直接用 `openaiText` 改 `baseURL` 就好。
</Callout>
</Step>
...
<Callout type="warning">
SSE 的 `Content-Type` 必須是 `text/event-stream`，`Cache-Control: no-cache` 也是必要的，否則瀏覽器可能不會即時顯示 streaming 內容。
</Callout>
</Step>
